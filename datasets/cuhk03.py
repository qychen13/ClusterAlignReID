...
class CUHK03(BaseImageDataset):
    ...
    def __init__(self, root, split_id=0, cuhk03_labeled=False,
        ...
        def process_info(s):
            temp = []
            for img_path, pid, camid in s:
                temp.append((img_path, dict(img_path=img_path, pid=pid, camid=camid)))
            return temp
        train = process_info(split['train'])
        query = process_info(split['query'])
        gallery = process_info(split['gallery'])
...
class CUHK03Detected(CUHK03):
    def __init__(self, root, **kwargs):
        super(CUHK03Detected, self).__init__(root, cuhk03_labeled=False, **kwargs)

class CUHK03Labeled(CUHK03):
    def __init__(self, root, **kwargs):
        super(CUHK03Labeled, self).__init__(root, cuhk03_labeled=True, **kwargs)